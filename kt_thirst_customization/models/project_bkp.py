...
class Project(models.Model):
    ...
    staffing_ids = fields.One2many('hr.employee','project_id',string="Staffing")
	...
	for item in vendor_list:
	    po_create = self.env['purchase.order'].create({'partner_id':item['vendor'],'order_line':[(0,0,{'product_id':product.id,'name':product.name,'date_planned':datetime.now(),'product_uom':product.uom_po_id.id,'product_qty':qty_rqd[product.id],'price_unit':product.standard_price}) for product in item['products']]})
	...
	for obj in self.search([]):
	    ...
	    if function_date:
		    ...
		    print 'obj=================',obj.sale_order_id.name
