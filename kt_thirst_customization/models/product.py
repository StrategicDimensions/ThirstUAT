        ...
        if self.next_transfer_type == 1:
                    ...
                    for bev_select_obj in self.beverages_selection_ids:
                       for obj in bev_select_obj.standard_beverage_ids:
                           ...
                           default_code_custom = product_obj.default_code.encode('ascii','ignore') if type(product_obj.default_code) == 'unicode' else str(product_obj.default_code)
                           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick_obj.id,
                                                                           ...
                                                                           'name':'['+default_code_custom+'] '+str(product_obj.name if product_obj.name else False),
                                                                           'date_expected':str(datetime.now())})
