...
class Project(models.Model):
    ...
    @api.multi
    def transfer_stock_to_function_locationnnnnn_oldddd(self):
        '''to create warehouse locations and internal transfer'''
	if not self.cage:
	    raise ValidationError('Please select Cage')
	if not self.vehicle:
	    raise ValidationError('Please select Vehicle')
	...
	stock_pick_obj.action_confirm()
        ...
        #creating internal transfer(stock operation/stock picking) Function internal location -> Cage location
        trans_vals = {  'partner_id':self.sale_order_id.partner_id.id,'picking_type_id':picking_type_obj.id,'priority':'1','move_type':'one',
                        'location_id':fn_loc_obj.id,'location_dest_id':self.cage.id,'min_date':str(event_start+timedelta(days=-1)),
                        'origin':self.project_number
                     }
        stock_pick2_obj = self.env['stock.picking'].create(trans_vals)
        for obj in self.consumable_beverage_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick2_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.qty_required,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick2_obj.location_id.id,'location_dest_id':stock_pick2_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_start+timedelta(days=-1))})

        for obj in self.equipment_beverage_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick2_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.qty_required,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick2_obj.location_id.id,'location_dest_id':stock_pick2_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_start+timedelta(days=-1))})

        for obj in self.product_bom_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick2_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.product_qty,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick2_obj.location_id.id,'location_dest_id':stock_pick2_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_start+timedelta(days=-1))})

        for bev_select_obj in self.beverages_selection_ids:
           for obj in bev_select_obj.selected_beverages_ids:
               product_obj = self.env['product.product'].search([('product_tmpl_id','=',obj.product_id.id)],limit=1)
               stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick2_obj.id,'product_id':product_obj.id,'product_uom_qty':obj.qty_required,'product_uom':product_obj.uom_id.id,'location_id':stock_pick2_obj.location_id.id,'location_dest_id':stock_pick2_obj.location_dest_id.id,'name':'['+str(product_obj.default_code)+'] '+str(product_obj.name),'date_expected':str(event_start+timedelta(days=-1))})
	stock_pick2_obj.action_confirm()
        ...
        stock_pick3_obj = self.env['stock.picking'].create(trans_vals)
        for obj in self.consumable_beverage_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick3_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.qty_required,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick3_obj.location_id.id,'location_dest_id':stock_pick3_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_start+timedelta(days=-1))})

        for obj in self.equipment_beverage_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick3_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.qty_required,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick3_obj.location_id.id,'location_dest_id':stock_pick3_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_start+timedelta(days=-1))})

        for obj in self.product_bom_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick3_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.product_qty,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick3_obj.location_id.id,'location_dest_id':stock_pick3_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_start+timedelta(days=-1))})

        for bev_select_obj in self.beverages_selection_ids:
           for obj in bev_select_obj.selected_beverages_ids:
               product_obj = self.env['product.product'].search([('product_tmpl_id','=',obj.product_id.id)],limit=1)
               stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick3_obj.id,'product_id':product_obj.id,'product_uom_qty':obj.qty_required,'product_uom':product_obj.uom_id.id,'location_id':stock_pick3_obj.location_id.id,'location_dest_id':stock_pick3_obj.location_dest_id.id,'name':'['+str(product_obj.default_code)+'] '+str(product_obj.name),'date_expected':str(event_start+timedelta(days=-1))})
	stock_pick3_obj.action_confirm()

        #creating internal transfer(stock operation/stock picking) Vehicle Location --> Function Event Location
        trans_vals = {  'partner_id':self.sale_order_id.partner_id.id,'picking_type_id':picking_type_obj.id,'priority':'1','move_type':'one',
                        'location_id':self.vehicle.id,'location_dest_id':fn_ven_loc_obj.id,'min_date':str(event_start),
                        'origin':self.project_number
                     }
        stock_pick3_obj = self.env['stock.picking'].create(trans_vals)
        for obj in self.consumable_beverage_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick3_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.qty_required,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick3_obj.location_id.id,'location_dest_id':stock_pick3_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_start)})
        ...
        for bev_select_obj in self.beverages_selection_ids:
           for obj in bev_select_obj.selected_beverages_ids:
               product_obj = self.env['product.product'].search([('product_tmpl_id','=',obj.product_id.id)],limit=1)
               stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick3_obj.id,'product_id':product_obj.id,'product_uom_qty':obj.qty_required,'product_uom':product_obj.uom_id.id,'location_id':stock_pick3_obj.location_id.id,'location_dest_id':stock_pick3_obj.location_dest_id.id,'name':'['+str(product_obj.default_code)+'] '+str(product_obj.name),'date_expected':str(event_start)})
	stock_pick3_obj.action_confirm()

        #creating incoming shipment/transfer(stock operation/stock picking) Function Event Location --> Vehicle Location
        trans_vals = {  'partner_id':self.sale_order_id.partner_id.id,'picking_type_id':picking_type_obj.id,'priority':'1','move_type':'one',
                        'location_dest_id':self.vehicle.id,'location_id':fn_ven_loc_obj.id,'min_date':str(event_end),
                        'origin':self.project_number
                     }
        stock_pick4_obj = self.env['stock.picking'].create(trans_vals)
        for obj in self.consumable_beverage_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick4_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.qty_required,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick4_obj.location_id.id,'location_dest_id':stock_pick4_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_end)})

        for obj in self.equipment_beverage_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick4_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.qty_required,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick4_obj.location_id.id,'location_dest_id':stock_pick4_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_end)})

        for obj in self.product_bom_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick4_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.product_qty,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick4_obj.location_id.id,'location_dest_id':stock_pick4_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_end)})

        for bev_select_obj in self.beverages_selection_ids:
           for obj in bev_select_obj.selected_beverages_ids:
               product_obj = self.env['product.product'].search([('product_tmpl_id','=',obj.product_id.id)],limit=1)
               stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick4_obj.id,'product_id':product_obj.id,'product_uom_qty':obj.qty_required,'product_uom':product_obj.uom_id.id,'location_id':stock_pick4_obj.location_id.id,'location_dest_id':stock_pick4_obj.location_dest_id.id,'name':'['+str(product_obj.default_code)+'] '+str(product_obj.name),'date_expected':str(event_end)})
	stock_pick4_obj.action_confirm()
        ...
        stock_pick5_obj = self.env['stock.picking'].create(trans_vals)
        for obj in self.consumable_beverage_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick5_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.qty_required,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick5_obj.location_id.id,'location_dest_id':stock_pick5_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_end)})

        for obj in self.equipment_beverage_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick5_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.qty_required,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick5_obj.location_id.id,'location_dest_id':stock_pick5_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_end)})

        for obj in self.product_bom_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick5_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.product_qty,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick5_obj.location_id.id,'location_dest_id':stock_pick5_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_end)})

        for bev_select_obj in self.beverages_selection_ids:
           for obj in bev_select_obj.selected_beverages_ids:
               product_obj = self.env['product.product'].search([('product_tmpl_id','=',obj.product_id.id)],limit=1)
               stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick5_obj.id,'product_id':product_obj.id,'product_uom_qty':obj.qty_required,'product_uom':product_obj.uom_id.id,'location_id':stock_pick5_obj.location_id.id,'location_dest_id':stock_pick5_obj.location_dest_id.id,'name':'['+str(product_obj.default_code)+'] '+str(product_obj.name),'date_expected':str(event_end)})
	stock_pick5_obj.action_confirm()

        #creating incoming shipment/transfer(stock operation/stock picking) Cage location --> Function internal location 
        trans_vals = {  'partner_id':self.sale_order_id.partner_id.id,'picking_type_id':picking_type_obj.id,'priority':'1','move_type':'one',
                        'location_dest_id':fn_loc_obj.id,'location_id':self.cage.id,'min_date':str(event_end+timedelta(days=1)),
                        'origin':self.project_number
                     }
        stock_pick6_obj = self.env['stock.picking'].create(trans_vals)
        for obj in self.consumable_beverage_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick6_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.qty_required,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick_obj.location_id.id,'location_dest_id':stock_pick_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_end+timedelta(days=1))})

        for obj in self.equipment_beverage_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick6_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.qty_required,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick_obj.location_id.id,'location_dest_id':stock_pick_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_end+timedelta(days=1))})

        for obj in self.product_bom_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick6_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.product_qty,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick6_obj.location_id.id,'location_dest_id':stock_pick6_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_end+timedelta(days=1))})

        for bev_select_obj in self.beverages_selection_ids:
           for obj in bev_select_obj.selected_beverages_ids:
               product_obj = self.env['product.product'].search([('product_tmpl_id','=',obj.product_id.id)],limit=1)
               stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick6_obj.id,'product_id':product_obj.id,'product_uom_qty':obj.qty_required,'product_uom':product_obj.uom_id.id,'location_id':stock_pick6_obj.location_id.id,'location_dest_id':stock_pick6_obj.location_dest_id.id,'name':'['+str(product_obj.default_code)+'] '+str(product_obj.name),'date_expected':str(event_end+timedelta(days=1))})
	stock_pick6_obj.action_confirm()

        #creating incoming shipment/transfer(stock operation/stock picking) function internal location --> WH/Stock
        trans_vals = {  'partner_id':self.sale_order_id.partner_id.id,'picking_type_id':picking_type_obj.id,'priority':'1','move_type':'one',
                        'location_dest_id':source_loc_obj.id,'location_id':fn_loc_obj.id,'min_date':str(event_end+timedelta(days=1)),
                        'origin':self.project_number
                     }
        stock_pick7_obj = self.env['stock.picking'].create(trans_vals)
        for obj in self.consumable_beverage_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick7_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.qty_required,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick7_obj.location_id.id,'location_dest_id':stock_pick7_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_end+timedelta(days=1))})

        for obj in self.equipment_beverage_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick7_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.qty_required,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick7_obj.location_id.id,'location_dest_id':stock_pick7_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_end+timedelta(days=1))})

        for obj in self.product_bom_ids:
           stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick7_obj.id,'product_id':obj.product_id.id,'product_uom_qty':obj.product_qty,'product_uom':obj.product_id.uom_id.id,'location_id':stock_pick7_obj.location_id.id,'location_dest_id':stock_pick7_obj.location_dest_id.id,'name':'['+str(obj.product_id.default_code)+'] '+str(obj.product_id.name),'date_expected':str(event_end+timedelta(days=1))})

        for bev_select_obj in self.beverages_selection_ids:
           for obj in bev_select_obj.selected_beverages_ids:
               product_obj = self.env['product.product'].search([('product_tmpl_id','=',obj.product_id.id)],limit=1)
               stock_move_obj = self.env['stock.move'].create({'picking_id':stock_pick7_obj.id,'product_id':product_obj.id,'product_uom_qty':obj.qty_required,'product_uom':product_obj.uom_id.id,'location_id':stock_pick7_obj.location_id.id,'location_dest_id':stock_pick7_obj.location_dest_id.id,'name':'['+str(product_obj.default_code)+'] '+str(product_obj.name),'date_expected':str(event_end+timedelta(days=1))})

	stock_pick7_obj.action_confirm()

	self.transfered_stock = True
	return True	
