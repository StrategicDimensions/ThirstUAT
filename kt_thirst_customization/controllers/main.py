...
class ProjAdditionalBudget(http.Controller):
    ...
    def project_additional_budget_new(self,proj_id,partner_id,**kw):
        request.session.update({'last_proj_id':proj_id,'last_project_id':proj_id,'for_additional_budget':True,'quotation_partner_id':partner_id})
	return request.redirect('/shop/product/proj-ab-additional-budget-32842')
...
class WebsiteSale(website_sale_main.WebsiteSale):
	...
	#Jagadeesh end
		...
		for inv in order.invoice_ids:
                    inv.inv_project_id = project_id
		request.env['beverage.budget'].sudo().create({'budget_amount':order.amount_total,'req_partner_id':order.partner_id.id,'date_req':str(datetime.now()),'project_id':project_id})
