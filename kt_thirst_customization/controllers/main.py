...
class ProjAdditionalBudget(http.Controller):
    @http.route("/additional/budget/<int:proj_id>/proj", type='http', auth="public", website=True)
    def project_additional_budget(self, proj_id, **kw):
        request.session.update({'last_proj_id': proj_id, 'last_project_id': proj_id, 'for_additional_budget': True})
        return request.redirect('/shop/product/proj-ab-additional-budget-32842')

    @http.route("/additional/budget/<int:proj_id>/<int:partner_id>/proj", type='http', auth="public", website=True)
    def project_additional_budget_new(self, proj_id, partner_id, **kw):
        request.session.update({'last_proj_id': proj_id, 'last_project_id': proj_id, 'for_additional_budget': True,
                                'quotation_partner_id': partner_id})
        return request.redirect('/shop/product/proj-ab-additional-budget-32842')
    ...
    def view(self, order_id, pdf=None, token=None, message=False, **post):
        ...
        if token:
            ...
            if Order and request.session.get('view_quote') != now:
                ...
                _message_post_helper(res_model='sale.order', res_id=Order.id, message=body, token=token,
                                     token_field="access_token", message_type='notification', subtype="mail.mt_note",
        ...
        request.session['go_for_additional_beverages'] = False  # Jagadeesh
        request.session.update({'for_additional_budget': False})
        days = 0
        if Order.validity_date:
            days = (fields.Date.from_string(Order.validity_date) - fields.Date.from_string(
                fields.Date.today())).days + 1
        if pdf:
            pdf = request.env['report'].sudo().with_context(set_viewport_size=True).get_pdf([Order.id],
        ...
        if Order.require_payment or values['need_payment']:
            values['acquirers'] = list(request.env['payment.acquirer'].sudo().search(
        ...
        values.update({'account_blocked': account_blocked, 'credit_limit_exceeded': credit_limit_exceeded,
                       'sale_rep': Order.user_id.name, 'quote_id': Order.id, 'token': token})
...
class PurchaseOrderQuote(http.Controller):
    ...
    def view(self, **post):
        ...
        return request.render('kt_thirst_customization.so_quotation_inherit',
                              {'quotation': sale_order, 'order_valid': True, 'message': 3})
...
class BeverageSelection(http.Controller):
    ...
    @http.route(['/beverage_selection/<project>/<selection_type>'], type='http', auth="public", website=True)
    def beverages_products_selection_custom(self, project, selection_type, **post):
        print '\n\nselection_type', selection_type
        request.session['view_mode'] = 'grid'
        return request.redirect('/beverages/%s/%s/products/all' % (project, selection_type))

    @http.route(['/beverages/<project>/<selection_type>/products/all'], type='http', auth="public", website=True)
    def products_all_selecton_custom(self, project, selection_type, **post):
        project_id = request.env['project.project'].browse(int(project))
        main_dict = defaultdict(list)
        beverage_menu_dict = {}
        for each in project_id.beverages_selection_ids:
            prod_list_1, prod_list_2 = [], []
            for each_menu_product in each.beverage_menu_id.beverage_product_ids:
                standard_prod_dict, premium_prod_dict, prod_dict = {}, {}, {}
                standard_prod_list, premium_prod_list = [], []
                if each_menu_product.prod_type == 'Standard':
                    standard_prod_list.append(each_menu_product)
                    standard_prod_dict.update({each_menu_product.sub_categ_id: standard_prod_list})
                    prod_list_1.append(standard_prod_dict)
                if each_menu_product.prod_type == 'Premium':
                    premium_prod_list.append(each_menu_product)
                    premium_prod_dict.update({each_menu_product.sub_categ_id: premium_prod_list})
                    prod_list_2.append(premium_prod_dict)
            beverage_menu_dict.update({each: {'standard': prod_list_1,
                                              'premium': prod_list_2}})

        result = {}
        for each, typedict in beverage_menu_dict.items():
            result.update({each: {}})
            for typ, itemlist in typedict.items():
                result[each].update({typ: {}})  # type wise
                for index in itemlist:
                    for key, value in index.items():
                        for valueindex in value:
                            if not result[each][typ].has_key(key):
                                result[each][typ].update({key: valueindex})
                            else:
                                result[each][typ][key] += valueindex

        active_menu_id = request.session.get('active_menu_id', 0)
        return request.render('kt_thirst_customization.beverage_selection_page_custom', 
            {'result': result, 'view_mode': 'grid', 'active_menu_id': active_menu_id})
    ...
    def products_all_selecton(self, bar, project, selection_type, **post):
        ...
        if True:
            ...
            if sale_order_id:
                ...
                if bar == 'full_bar' and 'Full Bar' in [line.product_id.name for line in order.order_line]:
                    full_bar_menu = request.env['beverages'].sudo().search([('name', '=', 'Full Bar Beverage Menu')],
                                                                           limit=1)
                    beverage_menu = full_bar_menu
                elif bar == 'cocktail_bar' and 'Cocktail Bar' in [line.product_id.name for line in order.order_line]:
                    cocktail_bar_menu = request.env['beverages'].sudo().search(
                        [('name', '=', 'Cocktail Bar Beverage Menu')], limit=1)
            ...
            if beverage_menu:
                ...
                for categ in request.env['beverages.category.rel'].sudo().search(
                        [('beverage_id', '=', beverage_menu.id)], order="sequence"):
                    category_seq.append(categ.categ_id.id)
                for categ in category_seq:
                    sub_categ_seq_dic.update({categ: []})
                for sub_categ in request.env['beverages.sub.category.rel'].sudo().search(
                        [('beverage_id', '=', beverage_menu.id)], order="sequence"):
                    for categ in category_seq:
                ...
                for categ in category_seq:
                    for sub_categ in sub_categ_seq_dic[categ]:
                        sub_categ_rel_obj = request.env['beverages.sub.category.rel'].sudo().search(
                            [('sub_categ_id', '=', sub_categ), ('beverage_id', '=', beverage_menu.id)])
                        if sub_categ in all_standard_products_dic.keys():
                            new_dic = {'categ_name': sub_categ_rel_obj.parent_categ.name,
                                       'sub_categ_name': sub_categ_rel_obj.sub_categ_id.name,
                                       'max_prod': sub_categ_rel_obj.max_products,
                                       'beverages': all_standard_products_dic[sub_categ]}
                ...
                for bev_products in all_standard_products_grid:
                    bev_products['beverages'] = [bev_products['beverages'][x:x + 5] for x in
                                                 xrange(0, len(bev_products['beverages']), 5)]
                # disable premium selection
        ...
        'selection_type':selection_type,
                ...
                return_dic = {'project_id': project_id, 'view_all': False,
                              'standard_selected_bevs': standard_bevs, 'premium_selected_bevs': False,
                              'total_premium_amount': False,
                              'finished': True, 'beverage_menu': beverage_menu.id, 'bar': bar, 'view_mode': view_mode,
                              'selection_type': selection_type,
                              'all_standard_products_list': all_standard_products,
                              'all_premium_products_list': False,
                              'all_standard_products_grid': all_standard_products_grid,
                              'all_premium_products_grid': False,
                              }
    ...
    @http.route(['/beverage_selection/complete/<project>'], type='http', auth="public", website=True)
    def beverage_selection_complete_custom(self, project, **post):
        print '\n\n\n mrethod called'
        project_id = int(project)
        ...
        # bev_select_obj = request.env['beverages.selection'].sudo().search(
        ...
        request.cr.execute(
            "update beverage_products set remove_button_visible = 'f' where remove_button_visible = 't'  ")
        # full_bar = False
        # shop = False
        # if bar == 'cocktail_bar':
        ...
        request.session.update(
            {'last_project_id': project_id, 'last_proj_id': project_id, 'go_for_additional_beverages': True,
             'for_additional_budget': False, 'quotation_partner_id': project_obj.sale_order_id.partner_id.id})
        return request.render('kt_thirst_customization.beverages_selection_complete_custom',
                              {'project_id': project_id})

    @http.route(['/beverage_selection/complete/<project>'], type='http', auth="public", website=True)
    def beverage_selection_complete(self, project, **post):
        ...
        request.session.update(
            {'last_project_id': project_id, 'last_proj_id': project_id, 'go_for_additional_beverages': True,
             'for_additional_budget': False, 'quotation_partner_id': project_obj.sale_order_id.partner_id.id})
        return request.render('kt_thirst_customization.beverages_selection_complete',
                              {'project_id': project_id})

    @http.route(['/add/<int:project_id>/<int:beverage_menu_id>/<product>'], type='http', auth="public", csrf=False, website=True)
    def add_product(self, project_id, beverage_menu_id, **post):
        project = request.env['project.project'].sudo().browse(project_id)
        if project.locked:
            return json.dumps({'message': 'Project is locked you can not add product'})
        products = []
        ...
        if post:
            ...
            if prod_type == 'Standard':
                bev_menu_obj = request.env['beverages'].sudo().browse(beverage_menu_id)
                sub_categ_obj = bev_menu_obj.beverage_sub_category_ids.filtered(
                    lambda x: x.sub_categ_id.id == int(sub_category_id))
                max_products = sub_categ_obj and sub_categ_obj.max_products or 0
                added_products = request.env['selected.beverages'].sudo().search(
                    [('sale_order_id', '=', sale_order_id), ('sub_category_id', '=', int(sub_category_id)),
                     ('bev_select_id.beverage_menu_id', '=', beverage_menu_id), ('prod_type', '=', 'Standard')])
                if len(added_products) >= max_products:
                    return json.dumps({'message': 'You cannot add more than the permitted products for the category', 'flag': 0})
            # creating record once we selected the beverage by click on select/purchase button
            bev_select_obj = request.env['beverages.selection'].sudo().search(
                [('project_id', '=', project_id), ('beverage_menu_id', '=', beverage_menu_id)])
            ...
            obj = request.env['selected.beverages'].sudo().create(
                {'sale_order_id': sale_order_id, 'category_id': sub_categ_obj.parent_categ.id,
                 'sub_category_id': sub_category_id, 'product_id': product_id, 'product_name': product_obj.name,
                 'product_code': product_obj.default_code, 'prod_type': prod_type,
                 'classification': product_obj.classification, 'on_hand': product_obj.qty_available,
                 'forecasted': product_obj.virtual_available, 'variance': -(product_obj.qty_available),
                 'bev_select_id': bev_select_obj.id})
            ...
            if obj.prod_type == 'Standard':
                ...
                dup_bev_prod = request.env['beverage.products'].sudo().search([('beverage_id', '=', beverage_menu_id),
                                                                               ('product_product_id', '=',
                                                                                bev_menu_prod_obj.product_product_id.id),
                                                                               ('category_id', '=',
                                                                                bev_menu_prod_obj.category_id.id), (
                                                                                   'sub_categ_id', '=',
                                                                                   bev_menu_prod_obj.sub_categ_id.id),
                                                                               ('prod_type', '=', 'Premium')], limit=1)
            elif obj.prod_type == 'Premium':
                obj.premium_bev_select_id = bev_select_obj.id
                # to get duplicated product from beverage menu products with diff product type
                dup_bev_prod = request.env['beverage.products'].sudo().search([('beverage_id', '=', ),
                                                                               ('product_product_id', '=',
                                                                                bev_menu_prod_obj.product_product_id.id),
                                                                               ('category_id', '=',
                                                                                bev_menu_prod_obj.category_id.id), (
                                                                                   'sub_categ_id', '=',
                                                                                   bev_menu_prod_obj.sub_categ_id.id),
                                                                               ('prod_type', '=', 'Standard')], limit=1)

            # to create object with duplicated beverae menu product
            if dup_bev_prod:
                product_obj = dup_bev_prod.product_product_id
                dup_selected_bev_obj = request.env['selected.beverages'].sudo().create(
                    {'sale_order_id': sale_order_id, 'category_id': sub_categ_obj.parent_categ.id,
                     'sub_category_id': sub_category_id, 'product_id': product_obj.id, 'product_name': product_obj.name,
                     'product_code': product_obj.default_code, 'prod_type': dup_bev_prod.prod_type,
                     'classification': product_obj.classification, 'on_hand': product_obj.qty_available,
                     'forecasted': product_obj.virtual_available, 'variance': -(product_obj.qty_available),
                     'bev_select_id': bev_select_obj.id})
            ...
            # end
        request.session['active_menu_id'] = beverage_menu_id
        return json.dumps({'message': 'Added to cart', 'flag': 1})

    @http.route(['/remove/<int:project_id>/<int:beverage_menu_id>/<product>'], type='http', auth="public", csrf=False, website=True)
    def remove_product(self, project_id, beverage_menu_id, **post):
        project = request.env['project.project'].sudo().browse(project_id)
        if project.locked:
            return json.dumps({'message': 'Project is locked you can not add product'})
        products = []
        ...
        if post:
            ...
            product_obj = request.env['product.template'].sudo().browse(int(product_id))
            sub_categ_obj = request.env['beverages.sub.category'].sudo().browse(int(sub_category_id))
            obj = request.env['selected.beverages'].sudo().search(
                [('sale_order_id', '=', sale_order_id), ('product_id', '=', int(product_id)),
                 ('bev_select_id', '=', bev_select_obj.id)])
...
class WebsiteSale(website_sale_main.WebsiteSale):
    ...
    def payment_confirmation(self, **post):
        ...
        project_id = request.session.get('last_project_id')
        if not project_id:
            project_id = request.session.get('last_proj_id')
        project_obj = request.env['project.project'].sudo().browse(project_id)
        ...
        cocktail_bar_menu_id = request.env['beverages'].sudo().search([('name', '=', 'Cocktail Bar Beverage Menu')],
                                                                      limit=1).id
        fullbar_bev_select_obj = request.env['beverages.selection'].sudo().search(
            [('project_id', '=', project_id), ('beverage_menu_id', '=', full_bar_menu_id)], limit=1)
        cocktail_bev_select_obj = request.env['beverages.selection'].sudo().search(
            [('project_id', '=', project_id), ('beverage_menu_id', '=', cocktail_bar_menu_id)], limit=1)
        # if fullbar_bev_select_obj and not fullbar_bev_select_obj.is_completed:
        #    full_bar = True
        if cocktail_bev_select_obj and not cocktail_bev_select_obj.is_completed:
            cocktail_bar = True
        elif fullbar_bev_select_obj and not fullbar_bev_select_obj.is_completed:
            full_bar = True
        # elif (cocktail_bev_select_obj and cocktail_bev_select_obj.is_completed) or (fullbar_bev_select_obj and fullbar_bev_select_obj.is_completed):
        #    shop = True
        ...
        if sale_order_id:
            order = request.env['sale.order'].sudo().browse(sale_order_id)
            if request.session.get('go_for_additional_beverages'):
                order.project_project_id = project_id
                for inv in order.invoice_ids:
                    inv.inv_project_id = project_id
                for line in order.order_line:
                    product_obj = line.product_id.product_tmpl_id
                    request.env['selected.beverages'].sudo().create(
                        {'sale_order_id': sale_order_id, 'product_id': product_obj.id, 'product_name': product_obj.name,
                         'product_code': product_obj.default_code, 'qty_required': line.product_uom_qty,
                         'prod_type': 'Premium', 'classification': product_obj.classification,
                         'on_hand': product_obj.qty_available, 'forecasted': product_obj.virtual_available,
                         'variance': -(product_obj.qty_available),
                         'bev_select_id': project_obj.beverages_selection_ids[0].id,
                         'premium_bev_select_id': project_obj.beverages_selection_ids[0].id})
            ...
            # Jagadeesh end
            if request.session.get('for_additional_budget'):
                order.project_project_id = project_id
                for inv in order.invoice_ids:
                    inv.inv_project_id = project_id
                request.env['beverage.budget'].sudo().create(
                    {'budget_amount': order.amount_total, 'req_partner_id': order.partner_id.id,
                     'date_req': str(datetime.now()), 'project_id': project_id})

            request.session['go_for_additional_beverages'] = False
            request.session['for_additional_budget'] = False
            return request.render("website_sale.confirmation",
                                  {'order': order, 'full_bar': full_bar, 'cocktail_bar': cocktail_bar,
                                   'project_id': project_id})
        else:
            return request.redirect('/shop')
