...
class BeverageSelection(http.Controller):
        ...
        @http.route(['/beverage_selection/<bar>/<project>'],type='http',auth="public", website=True)
        def beverage_selection(self,bar,project,**post):
		print 'barrrrrrrrrrrrrrrrrrrr test============'
                categories = []
                sub_categories = []
		bev_pages = {}
                page_count = 0
		...
		project_obj = request.env['project.project'].sudo().browse(project_id)
		sale_order_id = project_obj.sale_order_id.id
		request.session.update({'view_mode':'grid'})
			...
			if bar == 'full_bar' and 'Full Bar' in [line.product_id.name for line in order.order_line]:
				...
				for bev_select_obj in project_obj.beverages_selection_ids:
				    if bev_select_obj.beverage_menu_id.id == full_bar_menu.id and not bev_select_obj.selected_beverages_ids:
					project_obj.default_beverages_selection(bev_select_obj.id)
				#end
			        categories = full_bar_menu.beverage_category_ids
                                sub_categories = full_bar_menu.beverage_sub_category_ids
                            ...
                            if cocktail_bar_menu:
				#to auto select default products
                                for bev_select_obj in project_obj.beverages_selection_ids:
                                    if bev_select_obj.beverage_menu_id.id == cocktail_bar_menu.id and not bev_select_obj.selected_beverages_ids:
                                        project_obj.default_beverages_selection(bev_select_obj.id)
                                #end
                                categories = cocktail_bar_menu.beverage_category_ids
                                sub_categories = cocktail_bar_menu.beverage_sub_category_ids
                        for categ in categories:
                            for sub_categ in sub_categories:
                                if sub_categ.parent_categ.id == categ.categ_id.id:
                                    page_count += 1
                                    bev_pages.update({page_count:'/sub_categories/%s/%s/%s/%s'%(project_id,bar,sub_categ.sub_categ_id.id,categ.categ_id.id)})

                        if bev_pages:
			    bev_pages.update({0:'/beverage_selection/%s'%bar})
                            request.session.update({'beverage_pages':bev_pages,'bev_page_no':1})
                            return request.redirect(request.session['beverage_pages'][1])
                return request.render('kt_thirst_customization.beverage_selection_page',{'categories':categories,'sub_categories':sub_categories})


        @http.route(['/beverage_selection/prev'],type='http',auth="public", website=True)
        def beverage_selection_prev(self,**post):
	    if request.session['bev_page_no'] != 0:		
                request.session['bev_page_no'] -= 1
            url = request.session['beverage_pages'][request.session['bev_page_no']]
	    return request.redirect(url)

        @http.route(['/beverage_selection/next'],type='http',auth="public", website=True)
        def beverage_selection_next(self,**post):
            request.session['bev_page_no'] += 1
            url = request.session['beverage_pages'][request.session['bev_page_no']]
            return request.redirect(url)
	...
	@http.route(['/beverage_selection/<bar>/complete/<project>'],type='http',auth="public", website=True)
        ...
        @http.route(['/sub_categories/<project>/<bar>/<sub_categ>/<categ>'],type='http', auth="public", website=True)
        def sub_categories(self,project,bar,**post):
		products , standard_products ,premium_products = [],[],[]
		page_prev , page_next ,finished = True,True,False
                if request.session['bev_page_no'] == 1:
                    page_prev = False
                elif request.session['bev_page_no'] == len(request.session['beverage_pages']) - 1:
                    page_next = False
                    finished = True
		beverage_menu , full_bar_menu , cocktail_bar_menu = False,False,False
		view_mode = request.session['view_mode']
			...
			if beverage_menu:
			    categories = beverage_menu.beverage_category_ids
                            sub_categs = beverage_menu.beverage_sub_category_ids
			...
			''' getting total premium amount of selected beverages and displaying at web page.
			    filtering all selected premium beverages '''
			...
			request.session.update({'premium_products':{'sale_order_id':sale_order_id,'products':all_premium_products}})
                        categ_id = int(post.get('categ'))
                        sub_categ_id = int(post.get('sub_categ'))
                        categ_name = request.env['beverages.category'].sudo().browse(categ_id).name
                        sub_categ_name = request.env['beverages.sub.category'].sudo().browse(sub_categ_id).name
                        if sub_categ_id:
				''' getting all sub categories of parent category '''
				sub_categories = sub_categs.filtered(lambda x: x.parent_categ.id == categ_id)
				''' getting products based on respective parent and sub caterories'''
				if beverage_menu:
                                    for line in beverage_menu.beverage_product_ids:
                                        if line.sub_categ_id.id == sub_categ_id and line.category_id.id == categ_id:
                                            if line.prod_type == 'Standard':standard_products.append(line)
                                            else: premium_products.append(line)
			        ...
			        #To display 5 products for each row
				standard_products_list = [standard_products[x:x+5] for x in xrange(0, len(standard_products) ,5)]
     			        premium_products_list = [premium_products[x:x+5] for x in xrange(0,len(premium_products),5)]

                                return request.render('kt_thirst_customization.beverage_selection_page',{'project_id':project_id,'categories':categories,'sub_categories':sub_categories,'standard_products':standard_products_list,'premium_products':premium_products_list,'standard_selected_bevs':standard_bevs,'premium_selected_bevs':premium_bevs,'total_premium_amount':total_premium_amount,'categ_id':categ_id,'sub_categ_id':sub_categ_id,'categ_name':categ_name,'sub_categ_name':sub_categ_name,'page_prev':page_prev,'page_next':page_next,'finished':finished,'beverage_menu':beverage_menu.id,'bar':bar,'standard_products_list':standard_products,'premium_products_list':premium_products,'view_mode':view_mode,'view_all':True})
                        else:
                                return request.render('website.shop')
                return request.render('kt_thirst_customization.beverage_selection_page')
	...
	@http.route(['/add/<project>/<beverage_menu>/<product>'],type='http', auth="public",csrf=False,website=True)
		...
		count = 0
			...
			#to get duplicated product from beverage menu products with diff product type
                        dup_bev_prod = request.env['beverage.products'].sudo().search([('beverage_id','=',int(beverage_menu)),('product_product_id','=',bev_menu_prod_obj.product_product_id.id),('category_id','=',bev_menu_prod_obj.category_id.id),('sub_categ_id','=',bev_menu_prod_obj.sub_categ_id.id),('prod_type','=','Standard')],limit=1)#to get duplicated product from beverage menu products with diff product type
        ...
        @http.route(['/beverages/<bar>/<project>/products/all'],type='http', auth="public", website=True)
        def products_all(self,bar,project,**post):

                products , standard_products ,premium_products = [],[],[]
		view_mode = request.session['view_mode']
		print 'inside post=============',post
		return_dic = {}
                #if post:
		if True:
                        project_id = int(project)
				...
				all_standard_products,all_premium_products,all_standard_products_grid,all_premium_products_grid= [],[],[],[]
					...
					if line.sub_categ_id.name in all_standard_products_dic.keys():
					    ...
					    all_standard_products_dic.update({line.sub_categ_id.name:[line] })
	                            ...
	                            new_dic = {'categ_name':key,'beverages':all_standard_products_dic[key]}
        	                    all_standard_products.append(new_dic)
                	        for key in all_premium_products_dic.keys():
                        	    new_dic = {'categ_name':key,'beverages':all_premium_products_dic[key]}
